def procesar_credenciales(username, password, date):
    ...
    meas_list = [item['valueDouble'] for item in meas[0]]   
    resultado = f"Medidas: {meas_list}"
    return resultado
...
class Edistribucion():
    ...
    def __init__(self, u, p, login=None, password=None, debug_level=logging.INFO):
        credentials = Credentials_class(u, p)
